...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int w = 0; w < _width; w++) {
            for (int h = 0; h < _height; h++) {
                _solnNumToPlace[solution[w][h] - 1] = Place.pl(w, h);
            }
        }
        for (Place place:_solnNumToPlace) {
            int order = 0;
            for (order = 0; order < _solnNumToPlace.length; order++) {
                if (_solnNumToPlace[order] == place) {
                    break;
                }
            }
            int direction = 0;
            int sequenceNumber = 0;
            if (order + 1 != _width * _height) {
                direction = place.dirOf(_solnNumToPlace[order + 1]);
            }
            if (order > 0 && order < _width * _height - 1) {
                sequenceNumber = 0;
            } else {
            ...
            Sq sq = new Sq(
                    ...
                    place.y,
                    sequenceNumber,
                    order == 0 || order == _width * _height - 1,
                    direction,
                    -1
            );
            sq._predecessors = new PlaceList();
            _board[place.x][place.y] = sq;
            _allSquares.add(sq);
        }

        for (Place place:_solnNumToPlace) {
            Sq sq = get(place);
            sq._successors = _allSuccessors[place.x][place.y][sq._dir];
            for (Place lastPlace : sq._successors) {
                if (get(lastPlace)._predecessors == null) {
                    get(lastPlace)._predecessors = new PlaceList();
                }
                get(lastPlace)._predecessors.add(sq.pl);
            }
        }
        _unconnected = last - 1;
    }
    ...
    Model(Model model) {
        ...
        for (Sq sq : model) {
            int w = sq.x;
            int h = sq.y;
            _board[w][h] = model._board[w][h];
            _allSquares.add(_board[w][h]);
        }

        for (Place place : _solnNumToPlace) {
            Sq sq = get(place);
            if (model.get(place)._successor != null) {
                sq._successor = get(model.get(place)._successor.pl);
            }
            if (model.get(place)._predecessor != null) {
                sq._predecessor = get(model.get(place)._predecessor.pl);
            }
        }
    }
     ...
     * @param num as the argument to find the Sq that with this input number
     * and the return value will be
     * @return null.*/
    private Sq findSq(int num) {
        for (Sq sq: _allSquares) {
            if (sq._sequenceNum == num) {
                return sq;
            }
        }
        return null;
    ...
    boolean autoconnect() {
        ...
        for (int target = 1; target < (_width * _height); target++) {
            Sq sq = findSq(target);
            Sq sqn = findSq(target + 1);
            if (sq != null && sqn != null) {
                result = result || sq.connect(sqn);
            }
        }
        return result;
    }
    ...
    void solve() {
        ...
        for (Place place:_solnNumToPlace) {
            ...
            for (get(place)._sequenceNum = 0;
                 get(place)._sequenceNum < _solnNumToPlace.length;
                 get(place)._sequenceNum++) {
                if (_solnNumToPlace[get(place)._sequenceNum] == place) {
                    break;
                }
            }
            get(place)._sequenceNum++;
    ...
    private int arrowDirection(int x, int y) {
        ...
        if (_board[x][y] == null) {
            ...
            return Place.dirOf(_board[x][y].x, _board[x][y].y,
                    _board[x][y]._successor.x, _board[x][y]._successor.y);
        }
    }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            ...
            if (this._successor == null
                    ...
                    && this.successors().contains(s1.pl)) {
        ...
        boolean connect(Sq s1) {
            ...
            if (this._sequenceNum != 0) {
                int number = this._sequenceNum;
                Sq mySuccessor = this._successor;
                while (mySuccessor != null) {
                    mySuccessor._sequenceNum = number + 1;
                    mySuccessor = mySuccessor._successor;
                    number += 1;
                }
            }
            ...
            Sq mySuccessor = s1;
            while (mySuccessor != null) {
                mySuccessor._head = this._head;
                mySuccessor = mySuccessor._successor;
            }

            if (s1._sequenceNum != 0) {
                int number = s1._sequenceNum;
                Sq myPredecessor = this;
                while (myPredecessor != null) {
                    myPredecessor._sequenceNum = number - 1;
                    myPredecessor = myPredecessor._predecessor;
                    number -= 1;
                }
            }

            if (s1._sequenceNum == 0 && _sequenceNum == 0) {
                _head._group = joinGroups(sgroup, _head._group);
            }
