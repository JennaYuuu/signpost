...
class PuzzleGenerator implements PuzzleSource {
    ...
    /** helper function
     * private int[] getCoordinate(int direction){
     * return new int[]{ Place.DX[direction], Place.DY[direction]};
     * }*/
    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                ArrayList<Sq> nodes = new ArrayList<>();
                PlaceList successors = sq.successors();

                for (Place place: successors) {
                    if (sq.connectable(model.get(place))) {
                        nodes.add(model.get(place));
                        nFound++;
                    }
                }
                found = nodes.get(0);
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                ...
                for (Place place: predecessors) {
                    if (model.get(place).connectable(sq)) {
                        nodes.add(model.get(place));
                        nFound++;
                    }
                }
                found = model.get(nodes.get(0));
